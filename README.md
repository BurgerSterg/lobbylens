...
- Riot Client running (LobbyLens reads from it locally)
...
When signups reopen, get your key at `https://app.henrikdev.xyz`, then add it in LobbyLens under **Settings**.

---

## Installation

1. Go to [Releases](https://github.com/BurgerSterg/lobbylens/releases)
2. Download `lobbylens_x64-setup.exe`
3. Run the installer
4. Launch LobbyLens from your Start menu or desktop
...
4. Click **Auto** — LobbyLens will automatically detect when you enter agent select or a live match and load the lobby
...
- Match history only records matches played while LobbyLens is running, plus a one-time backfill of past matches on first launch (requires Henrik key)

---

## Notes

LobbyLens reads data from your local Riot Client and the Henrik Dev API. It does not modify any game files or interact with Valorant's servers in any way that would violate terms of service. Use at your own discretion.
